...
REPORT_NUMBER=${INPUT_GITHUB_RUN_NUM}
...
if [[ ${INPUT_SUBFOLDER} != '' ]]; then
    ...
    REPORT_NUMBER=$(($(cat ${INPUT_ALLURE_HISTORY}/index.html | sed -nr "s/.*${INPUT_SUBFOLDER}\/([0-9]+)\/.*/\1/p")+1))
fi
...
#echo "index.html"
echo "<!DOCTYPE html><meta charset=\"utf-8\"><meta http-equiv=\"refresh\" content=\"0; URL=${GITHUB_PAGES_WEBSITE_URL}/${REPORT_NUMBER}/\">" > ./${INPUT_ALLURE_HISTORY}/index.html # path
...
echo "\"reportUrl\":\"${GITHUB_PAGES_WEBSITE_URL}/${REPORT_NUMBER}/\"," >> executor.json
echo "\"buildUrl\":\"https://github.com/${INPUT_GITHUB_REPO}/actions/runs/${INPUT_GITHUB_RUN_ID}\"," >> executor.json
echo "\"buildName\":\"GitHub Actions Run #${INPUT_GITHUB_RUN_ID}\",\"buildOrder\":\"${REPORT_NUMBER}\"}" >> executor.json
#cat executor.json
...
echo "copy allure-report to ${INPUT_ALLURE_HISTORY}/${REPORT_NUMBER}"
cp -r ./${INPUT_ALLURE_REPORT}/. ./${INPUT_ALLURE_HISTORY}/${REPORT_NUMBER}
